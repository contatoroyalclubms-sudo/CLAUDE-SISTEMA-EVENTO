...
exports.TorreSupremaDashboard = void 0;
const react_1 = __importStar(require("react"));
const lucide_react_1 = require("lucide-react");
const TorreSupremaDashboard = ({ className = '' }) => {
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            ...
            case 'busy':
            case 'processing':
                return <lucide_react_1.Clock className="w-4 h-4 animate-spin"/>;
            case 'error':
            case 'failed':
    ...
    return (<div className={`min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white p-6 ${className}`}>
      {/* Header */}
      <div className="mb-8">
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center">
              <lucide_react_1.Activity className="w-6 h-6 text-blue-400"/>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 rounded-full bg-yellow-500/20 flex items-center justify-center">
              <lucide_react_1.Zap className="w-6 h-6 text-yellow-400"/>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center">
              <lucide_react_1.TrendingUp className="w-6 h-6 text-green-400"/>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 rounded-full bg-purple-500/20 flex items-center justify-center">
              <lucide_react_1.Server className="w-6 h-6 text-purple-400"/>
            </div>
          </div>
          <div className="mt-2">
            <div className="w-full bg-gray-700 rounded-full h-2">
              <div className="bg-gradient-to-r from-blue-500 to-purple-500 h-2 rounded-full transition-all duration-300" style={{ width: `${metrics.systemLoad}%` }}></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-gray-800/50 rounded-xl border border-gray-700 p-6">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold flex items-center">
              <lucide_react_1.Users className="w-5 h-5 mr-2 text-blue-400"/>
              Supreme Agents
            </h2>
          ...
          <div className="space-y-4">
            {agents.map((agent) => (<div key={agent.id} className="bg-gray-900/50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${agent.status === 'busy' ? 'bg-yellow-400' :
                agent.status === 'idle' ? 'bg-green-400' : 'bg-red-400'}`}></div>
                    <div>
                ...
                <div className="mt-3">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <div className="bg-gradient-to-r from-green-500 to-blue-500 h-2 rounded-full" style={{ width: `${agent.performance.successRate}%` }}></div>
                  </div>
                </div>
              </div>))}
          </div>
        ...
        <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-6">
          <h2 className="text-xl font-bold flex items-center mb-6">
            <lucide_react_1.Brain className="w-5 h-5 mr-2 text-purple-400"/>
            Live Activity
          </h2>
          
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {realTimeData.map((activity) => (<div key={activity.id} className="flex items-start space-x-3 p-3 rounded-lg bg-gray-900/30">
                <div className={`w-2 h-2 rounded-full mt-2 ${activity.type === 'success' ? 'bg-green-400' :
                activity.type === 'warning' ? 'bg-yellow-400' : 'bg-blue-400'}`}></div>
            ...
            {realTimeData.length === 0 && (<div className="text-center py-8 text-gray-500">
                <lucide_react_1.Activity className="w-8 h-8 mx-auto mb-2 opacity-50"/>
      ...
      <div className="mt-8">
        <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-6">
          <h2 className="text-xl font-bold flex items-center mb-6">
            <lucide_react_1.Globe className="w-5 h-5 mr-2 text-green-400"/>
            MCP Integration Status
          </h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {mcpServers.map((server) => (<div key={server.id} className="bg-gray-900/50 rounded-lg p-4">
                ...
                <div className="mt-3">
                  <div className="flex space-x-1">
                    {[1, 2, 3, 4, 5].map((bar) => (<div key={bar} className={`h-2 flex-1 rounded ${server.status === 'connected' && server.responseTime < 50 * bar
                    ? 'bg-green-400'
      ...
      <div className="mt-8">
        <div className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 rounded-xl border border-blue-700/50 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold flex items-center">
              <lucide_react_1.Rocket className="w-5 h-5 mr-2 text-blue-400"/>
...
exports.TorreSupremaDashboard = TorreSupremaDashboard;
exports.default = exports.TorreSupremaDashboard;
